...
internal sealed class CartCheckoutItemConfiguration : IEntityTypeConfiguration<CartCheckoutItem>
{
    public void Configure(EntityTypeBuilder<CartCheckoutItem> builder)
    {
        builder.ToTable("CartCheckoutItems");
        builder.HasKey(item => item.Id);
        builder.Property(item => item.Id).ValueGeneratedNever();
        builder.Property<Guid>("CartCheckoutId").IsRequired();
        builder.Property(item => item.ProductId).IsRequired();
        builder.HasIndex("CartCheckoutId", nameof(CartCheckoutItem.ProductId)).IsUnique();
